...
const UpdateQuestionForMcq = () => {
  ...
  const handleSubmit = (e: any) => {
    ...
    if (que === "") return toast.error("Question  is required");
      ...
      try {
        // post question to db
        axios
          .put(`http://localhost:10001/api/update-question/${id}`, {
            updatedQuestion,
            token,
          })
          .then((response) => {
  ...
  useEffect(() => {
    try {
      setLoading(true);
      axios
        .get("http://localhost:10001/api/getallsubjects")
        .then((response) => {
  ...
  useEffect(() => {
    if (selectedSubject !== "") {
      ...
      try {
        axios
          .get(
            `http://localhost:10001/api/getallchaptersbysubject/${selectedSubject}`
          )
          .then((response) => {
