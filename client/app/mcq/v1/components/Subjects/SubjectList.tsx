...
const SubjectList = () => {
  ...
  useEffect(() => {
    if (selectedSubjectForQuiz !== "") {
      ...
      axios
        .get(
          `http://localhost:100001/mcq/getallchaptersbysubject/${selectedSubjectForQuiz}`
        )
        .then((data) => {
          ...
          if (data.data.length > 0) {
            axios
              .get(
                `http://localhost:100001/mcq/getallquestionsbysubjectandchapter/${selectedSubjectForQuiz}/${data.data[0].name}`
              )
              .then((response) => {
                try {
